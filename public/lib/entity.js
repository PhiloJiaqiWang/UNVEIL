class Entity {
  ...
  PointIn(tileX, tileY, tileMaxX, tileMaxY) {
    let yOffset = 0
    let xOffset = 0
    if (key_list.indexOf("w") !== -1) { yOffset = 0 }
    if (key_list.indexOf("a") !== -1) { xOffset = -0.5, yOffset = .2 }
    if (key_list.indexOf("d") !== -1) { xOffset = +0.5, yOffset = .2 }
    if (key_list.indexOf("s") !== -1) { yOffset = +0.5 }
  ...
  ifClue(clue) {
    ...
    if (clueTemp) {
      ...
      image(glass, this.position.x - 10, player.position.y - 15, 20, 20);
      if (ifCheck){
        return ifCheck
      }else{
        return false
  ...
  move(blockList, tileSize) {
    ...
    if (key_list.indexOf("s") !== -1 ||key_list.indexOf("a") !== -1 || key_list.indexOf("w") !== -1|| key_list.indexOf("d") !== -1) {
